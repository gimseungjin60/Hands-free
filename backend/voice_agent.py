...
import config
from part_info import get_part_info, board_context, get_board
...
SYSTEM_PROMPT = (
    ...
    "- 기본은 간결한 답이다. 복창은 get_part_info 결과 confusable=true 일 때만 한다: 답 첫머리에 "
    "그 레퍼런스 번호를 또박또박 한 번 복창해 학생이 잘못 들린 걸 잡게 한다(예: 'R13, 알 십삼 맞지? …'). "
    "confusable=false 거나 학생이 값·지시어(예: '십 킬로옴', '전해 콘덴서')로 물었으면 복창하지 말고 바로 답한다.\n"
...
# 입력 발화에 영숫자 레퍼런스(R5, R13, AR1, TR5, BZ1, U4, C21 …)가 들어있는지.
# 복창은 이게 True 일 때만 — 값·지시어('십 킬로옴', '전해 콘덴서')로 물으면 복창 안 함.
_REF_RE = re.compile(r"[A-Za-z]{1,3}\s*-?\s*\d+")


def _utterance_has_ref(text: str) -> bool:
    return bool(_REF_RE.search(text or ""))


def _part_card(r: dict) -> dict | None:
    """get_part_info 결과 → 프론트 PartCard용 간결 dict(위치/값/색띠/극성)."""
    if not r.get("found"):
        return None
    card = {"reference": r.get("reference", "")}
    val = r.get("value") or r.get("spec")
    if val:
        card["value"] = val
    bands = r.get("bands")
    if bands:
        card["colorBands"] = "-".join(bands)
    if r.get("polarity"):
        if r.get("plus"):
            p = r["plus"]
            card["polarity"] = p if p.lstrip().startswith("+") else f"+ {p}"
        elif r.get("minus"):
            m = r["minus"]
            card["polarity"] = m if m.lstrip().startswith("-") else f"- {m}"
        else:
            card["polarity"] = r.get("orient") or r.get("pin1") or r.get("mark")
    return card
...
def _stt_prompt() -> str:
    """Whisper 어휘 바이어싱 프롬프트(보드 레퍼런스 + 도메인 용어).
    한국어 STT가 'R5'·'BZ1' 같은 영숫자 레퍼런스를 영문 대문자+숫자로 받아쓰도록 유도한다.
    (whisper-1 prompt 파라미터 — 모델/비용 불변, 인식 품질만 개선.)"""
    global _STT_PROMPT
    if _STT_PROMPT is None:
        refs = ", ".join(get_board().get("parts", {}).keys())
        _STT_PROMPT = (
            "납땜 실습 부품 질문. 부품 레퍼런스는 영문 대문자+숫자로 표기한다"
            "(R5, R13, D1, C2, U4, AR1, TR5, BZ1, X1, L1). "
            f"가능한 레퍼런스: {refs}. "
            "자주 쓰는 말: 저항, 콘덴서, 전해 콘덴서, 다이오드, 어레이 저항, "
            "트랜지스터, 부저, 극성, 색띠, 봉투."
        )
    return _STT_PROMPT
...
class VoiceAgent:
    def __init__(self, device_id: str = "default"):
        ...
        self.current_part = None       # 마지막 조회 부품 카드(프론트 PartCard용)
            ...
            for _ in range(4):  # 함수콜 루프 가드
                ...
                for tc in msg.tool_calls:
                    if tc.function.name == "get_part_info":
                        ...
                        # 복창은 학생이 영숫자 레퍼런스를 직접 말했을 때만.
                        # 값/지시어로 물어 GPT가 레퍼런스로 해석한 경우엔 억제(간결 우선).
                        if result.get("confusable") and not _utterance_has_ref(text):
                            result["confusable"] = False
                            result.pop("confusable_note", None)
                        if result.get("found"):
                            self.current_part = _part_card(result)
                    else:
